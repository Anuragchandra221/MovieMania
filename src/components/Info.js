    ...
    if (castnCrew.cast && castnCrew.crew){
        return (
            <div style={{width: 'fit-content'}} className="mx-auto mt-5 pt-2">
                {<p style={{color: '#FFAD00', width: 'fit-content'}}>
                <span className="mr-2">Directors: </span> 
                    {director.map((index, i)=>{
                        return i<4?<span key={`${i}b`} className="mr-3" style={{color: '#fff'}}>{index}</span> :<></>
                    })}
                </p>}
                <p className=" mr-3" style={{color: '#FFAD00', width: 'fit-content'}}>
                    <span className="mr-2">Writers: </span> 
                    {screenplay.map((index, i)=>{
                        return i<4?<span key={`${i}a`} className="mr-3" style={{color: '#fff'}}>{index}</span> :<></>
                    })}
                </p>
